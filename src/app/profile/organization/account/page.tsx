...
const App: React.FC = () => {
  ...
  const [notificationsEnabled, setNotificationsEnabled] =
    useState<boolean>(true); // State for notifications toggle
  ...
  const AUTH0_DOMAIN = process.env.NEXT_PUBLIC_AUTH0_ISSUER_BASE_URL?.replace(
    /^https?:\/\//,
    ""
  );
  ...
  const handleChangePassword = async () => {
    const AUTH0_DOMAIN = process.env.NEXT_PUBLIC_AUTH0_ISSUER_BASE_URL;
    const AUTH0_CLIENT_ID = process.env.NEXT_PUBLIC_AUTH0_CLIENT_ID;
    const userEmail = email; // from Auth0 user object

    if (!AUTH0_DOMAIN || !AUTH0_CLIENT_ID || !userEmail) {
      alert("Missing Auth0 config or user email.");
      return;
    }

    try {
      const res = await fetch(`${AUTH0_DOMAIN}/dbconnections/change_password`, {
        method: "POST",
      ...
      if (res.ok) {
        alert(responseText); // Shows success message
      } else {
        alert(responseText); // Shows error message
      }
    } catch (err) {
  ...
  const handleSubmitIncident = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message ||
            `Failed to submit incident report. Status: ${response.status}`
        );
      }
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans pt-10">
      ...
      <div className="w-full max-w-3xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden p-4 sm:p-6 lg:p-10 pb-20 md:pb-0 relative z-10">
        {/* Header */}
        ...
        <Tab.Group selectedIndex={activeTab} onChange={setActiveTab}>
          <Tab.List className="flex p-2 space-x-1 bg-gray-50 rounded-xl m-4 border border-gray-200">
            {["General", "Security", "Terms", "Contact"].map(
              (category, index) => (
                <Tab
                  key={category}
                  className={({ selected }) =>
                        ...
                        <input
                          ...
                          value={
                            showUserId
                              ? user?.sub || ""
                              : user?.sub
                              ? "*".repeat(user.sub.length)
                              : ""
                          }
                          readOnly
                          className="border border-gray-300 rounded-md px-3 py-2 bg-gray-50 text-gray-700 cursor-not-allowed w-full break-all"
                          style={{ fontFamily: "monospace", textAlign: "center" }}
                        />
                        <button
                          type="button"
                          onClick={() => setShowUserId((prev) => !prev)}
                          className="p-2 bg-blue-600 rounded hover:bg-blue-500 flex items-center"
                          aria-label={
              ...
              >
                ...
                <Disclosure as="div" className="mt-2">
                  {({ open }) => (
                    <>
                      <Disclosure.Button
                        className={`flex w-full justify-between rounded-lg bg-gray-50 px-4 py-3 text-left text-sm font-medium text-[${primaryColor}] hover:bg-gray-100 focus:outline-none focus-visible:ring focus-visible:ring-[${primaryColor}] focus-visible:ring-opacity-75 shadow-sm border border-gray-200`}
                      >
                      ...
                      <AnimatePresence>
                        {open && (
                          ...
                          >
                            <p className="mb-4">
                              It's a good practice to change your password
                ...
                <Disclosure as="div" className="mt-2">
                  {({ open }) => (
                    <>
                      <Disclosure.Button
                        className={`flex w-full justify-between rounded-lg bg-gray-50 px-4 py-3 text-left text-sm font-medium text-red-600 hover:bg-gray-100 focus:outline-none focus-visible:ring focus-visible:ring-red-500 focus-visible:ring-opacity-75 shadow-sm border border-gray-200`}
                      >
                        <span className="flex items-center">
                        ...
                        {open && (
                          ...
                          >
                            <p className="mb-4 text-red-700 font-semibold">
                              Warning: Deleting your account is irreversible and
                              will remove all your data.
                            </p>
                            <button className="px-4 py-2 bg-red-600 text-white font-semibold rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition duration-200">
                              Delete Account
                            </button>
                          </Disclosure.Panel>
                        )}
                    ...
                    <>
                      ...
                      <AnimatePresence>
                        {open && (
                          ...
                          >
                            <p className="mb-2">
                              Welcome to our service! By accessing or using our
                              website, you agree to be bound by these Terms of
                              Use and all applicable laws and regulations. If
                              you do not agree with any of these terms, you are
                              prohibited from using or accessing this site.
                            </p>
                            <p className="mb-2">
                              The materials contained in this website are
                              protected by applicable copyright and trademark
                              law. We reserve the right to revise these terms of
                              use for its website at any time without notice. By
                              using this website you are agreeing to be bound by
                              the then current version of these Terms and
                              Conditions of Use.
                            </p>
                            <p>
                              Any claim relating to our website shall be
                              governed by the laws of the State of [Your
                              State/Country] without regard to its conflict of
                              law provisions.
                            </p>
                          </Disclosure.Panel>
                        )}
                ...
                <Disclosure as="div" className="mt-2">
                  {({ open }) => (
                    <>
                      ...
                      >
                        <span className="flex items-center">
                          <FileText className="w-5 h-5 mr-2" /> Legal
                          Information
                        </span>
                        <ChevronUp
                          className={`${
                        ...
                        {open && (
                          ...
                          >
                            <p className="mb-2">
                              ...
                              our legal policies, including privacy policy,
                              cookie policy, and data protection regulations.
                              For detailed information, please refer to the
                              specific policy documents linked below.
                            </p>
                            <ul className="list-disc list-inside space-y-1">
                              ...
                              <li>
                                Cookie Policy: Explains the use of cookies on
                                our website.
                              </li>
                              <li>
                                GDPR Compliance: Information regarding our
                ...
                <Disclosure as="div" className="mt-6">
                  {({ open }) => (
                    <>
                      ...
                      >
                        <span className="flex items-center">
                          <AlertCircle className="w-5 h-5 mr-2" /> Report an
                          Incident
                        </span>
                        <ChevronUp
                          className={`${
                          ...
                          >
                            ...
                            {incidentResponse && (
                              <div className="mt-4 p-3 bg-green-100 text-green-800 rounded-md flex items-center">
                                ...
                                <span>
                                  Incident submitted successfully! Incident ID:{" "}
                                  {incidentResponse.id}
                                </span>
                              </div>
                            )}
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <p className="text-sm text-gray-600 flex items-center">
                                          Status:
                                          <span
                                            className={`ml-2 px-2 py-0.5 rounded-full text-xs font-semibold ${statusColorClass}`}
                                          >
