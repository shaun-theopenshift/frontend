...
import { api } from "@/utils/api"; // Assuming api.ts exists in utils
...
import { templateBase64 } from "../../../components/templateBase64";

// Headless UI Imports
import { Tab, Transition, Menu, RadioGroup } from "@headlessui/react";
    ...
    <div className="relative w-full">
      ...
      {showSuggestions &&
        ...
        ReactDOM.createPortal(
          <div
            className="z-[9999] bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto"
            style={{
              position: "absolute",
            ...
            {isLoading ? (
              ...
              <ul className="divide-y divide-gray-200">
                {suggestions.map((suggestion, index) => {
                  ...
                  return (
                    <li
                      key={`${suggestion.city}-${suggestion.state}-${suggestion.postcode}-${index}`}
                      className="px-4 py-3 hover:bg-gray-50 cursor-pointer text-sm text-gray-800 transition-colors duration-150"
                      onClick={() => {
                        onChange(suggestion.formatted || display);
                        setInput(suggestion.formatted || display);
...
export default function ManageJobsPage() {
  const [activeTab, setActiveTab] = useState<
    "active" | "history" | "timesheet" | "payments"
  >("active");
  ...
  const [selectedState, setSelectedState] = useState<
    keyof typeof AU_STATES_SUBURBS | ""
  >("");
  ...
  const [applicantProfiles, setApplicantProfiles] = useState<{
    [userId: string]: { fname: string; lname: string } | null;
  }>({});
  ...
  // Close dropdown when clicking outside (This is now handled by Headless UI Menu)
  // useEffect(() => {
  ...
  const activeJobsList = activeJobs.filter(
    (job: any) => !job.status || nonHistoryStatuses.includes(job.status)
  );

  const timesheetHistoryJobs = activeJobs.filter(
    (job: any) =>
  ...
  const canceledJobs = activeJobs.filter(
  ...
  const refreshApplicantsForVisibleJobs = async () => {
    if (expandedApplicantsJobId) {
      try {
        ...
        setApplicants((a) => ({
          ...a,
          [expandedApplicantsJobId]: Array.isArray(data)
            ? data
            : data.items || [],
        }));
      } catch (e) {
  ...
  const handleRespondToApplicant = async (
    requestId: number,
    approve: boolean,
    jobId: number
  ) => {
  ...
  React.useEffect(() => {
    ...
    expandedApplicantsJobId,
    expandedApplicantsJobId ? applicants[expandedApplicantsJobId] : undefined,
  ]);
  ...
  // New function to handle "Proceed to Pay"
  const handleProceedToPay = async (jobId: number) => {
    setPaymentActionLoading((l) => ({ ...l, [jobId]: true }));
    setPaymentActionError((s) => ({ ...s, [jobId]: "" }));
    try {
      // This API call needs to be added to your api.ts file
      const response = await api.createBookingPaymentSession(jobId);
      if (response && response.url) {
        router.push(response.url); // Redirect to the payment URL
      } else {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col md:flex-row">
      ...
      <main className="flex-1 md:pl-72 pt-8 px-2 sm:px-6">
        <div className="max-w-5xl mx-auto py-4 sm:py-8 pb-28 md:pb-0">
          <AnimatePresence>
            {showSuccessMessage && (
              <motion.div
          ...
          {!showJobForm ? (
            <>
              ...
              <Tab.Group
                ...
                onChange={(index) =>
                  setActiveTab(
                    ["active", "history", "timesheet", "payments"][index] as any
                  )
                }
              >
                    ...
                    {activeJobsList.length === 0 ? (
                      <div className="py-8 text-center text-gray-500 flex flex-col items-center justify-center">
                        <div className="w-64 h-64">
                          <LottieAnimation
                            animationData={jobHistoryAnimation}
                            loop={true}
                            autoplay={true}
                          />
                        </div>
                      ...
                      <div className="w-full overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200 shadow overflow-visible border-b border-gray-200 sm:rounded-lg ">
                          <thead className="bg-gray-50">
                            <tr>
                              ...
                              <th
                                scope="col"
                                className="relative px-6 py-3 text-gray-500"
                              >
                              ...
                              return (
                                <React.Fragment key={job.id}>
                                  <tr>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                      <button
                                        className={`font-semibold underline hover:text-[#2954bd] transition ${
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                      ...
                                      >
                                        <div>
                                          <Menu.Button className="flex items-center text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                            ...
                                            <div className="space-y-4">
                                              {applicantList.map(
                                                (applicant: any) => {
                                                  ...
                                                  const jobForApplicant =
                                                    activeJobsList.find(
                                                      (j: any) =>
                                                        j.id ===
                                                        applicant.booking_id
                                                    ) ||
                                                    historyJobsList.find(
                                                      (j: any) =>
                                                        j.id ===
                                                        applicant.booking_id
                                                    );
                                                  ...
                                                  return (
                                                    <div
                                                      key={applicant.id}
                                                      className="bg-white border rounded-lg shadow p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2"
                                                    >
                                                      <div>
                          ...
                          >
                            <div className="flex-1">
                              <div className="mb-2 flex flex-col sm:flex-row sm:items-center sm:gap-4">
                                <span className="font-bold text-lg text-[#2954bd]">
                                  {job.title}
                                </span>
                          ...
                          .map((job) => (
                            ...
                            >
                              ...
                              <div className="flex flex-col gap-2 min-w-[160px] items-end">
                                ...
                                {job.status === "pending_payment" && (
                                  <button className="px-4 py-2 rounded bg-blue-600 text-white font-semibold hover:bg-blue-700 transition mt-2">
                                    Proceed to Pay
                                  </button>
                                )}
                                <button
                                ...
                                <button
                                  ...
                                  onClick={async () => {
                                    setTimesheetActionLoading((l) => ({
                                      ...l,
                                      [job.id]: true,
                                    }));
                                    setTimesheetActionSuccess((s) => ({
                                      ...s,
                                      [job.id]: "",
                                    }));
                                    try {
                                      const amount = getAmountForJob(job);
                                      await api.approveTimesheet(
                                        job.id,
                                        true,
                                        amount
                                      );
                                      setTimesheetActionSuccess((s) => ({
                                        ...s,
                                        [job.id]:
                                          "Timesheet was approved successfully.",
                                      }));
                            ...
                            >
                              ...
                              <div className="flex flex-col gap-2 min-w-[160px] items-end">
                                ...
                                <button
                                  ...
                                  onClick={() => handleProceedToPay(job.id)}
                                >
            ...
            <AnimatePresence mode="wait">
              <motion.div
                key={step} // Key changes on step change to trigger animation
              ...
              >
                    ...
                    <li className="flex items-center gap-3">
                      <div
                        className={`rounded-full w-8 h-8 flex items-center justify-center font-bold border-2 ${
                          step === 1
                            ? "bg-[#2954bd] text-white border-[#2954bd]"
                            : "bg-white text-[#2954bd] border-[#2954bd]"
                        }`}
                      >
                        1
                      </div>
                ...
                <section className="flex-1 w-full max-w-2xl mx-auto ">
                  {step === 1 && (
                    <>
                      <div className="flex items-center justify-between mb-6 flex-wrap gap-2">
                        <h1 className="text-2xl sm:text-3xl font-bold text-black border-2 border-[#2954bd] rounded px-4 py-2">
                          Service Types
                        </h1>
                        <button
                          className="text-[#2954bd] border border-[#2954bd] rounded px-4 py-1 font-semibold hover:bg-[#2954bd]/10 transition text-sm"
                          onClick={() => setShowJobForm(false)}
                        >
                          Exit
                        </button>
                      </div>
                      <p className="font-semibold text-lg mb-6 text-black/100">
                        ...
                        {SERVICE_TYPES.map((service) => {
                          return (
                            <RadioGroup.Option
                              ...
                              value={service.key}
                              className={({ checked }) =>
                                `border rounded-xl p-5 flex flex-col sm:flex-row items-start gap-4 cursor-pointer transition shadow-sm ${
                                  checked
                                    ? "border-[#2954bd] bg-[#e6f2f2]/60"
                            ...
                            >
                              {({ checked }) => (
                                <div className="flex items-center pt-1">
                                    ...
                                    <div className="flex items-center gap-2 mb-1">
                                      ...
                                      {service.badge && (
                                        <span className="ml-2 px-2 py-0.5 rounded bg-yellow-100 text-yellow-800 text-xs font-semibold flex items-center gap-1">
                                          <CheckCircleIcon className="w-4 h-4 inline-block text-yellow-600" />{" "}
                                          {service.badge}
                                        </span>
                                      )}
                                    </div>
                                  ...
                                  {checked && ( // Only show expanded description if selected
                  ...
                  {step === 2 && (
                    <>
                      <div className="flex items-center justify-between mb-6 flex-wrap gap-2">
                        <h1 className="text-2xl sm:text-3xl font-bold text-black border-2 border-[#2954bd] rounded px-4 py-2 flex items-center gap-2">
                          <MapPinIcon className="w-7 h-7 text-[#2954bd]" />
                          Location
                        </h1>
                        <button
                          className="text-[#2954bd] border border-[#2954bd] rounded px-4 py-1 font-semibold hover:bg-[#2954bd]/10 transition text-sm"
                      ...
                      {selectedState && (
                        <div className="mb-6">
                          ...
                          >
                            ...
                            {(selectedState
                              ? AU_STATES_SUBURBS[selectedState]
                              : []
                            ).map((s: string) => (
                      ...
                      <div className="mb-6">
                        ...
                        <p className="text-gray-600 mb-2">
                          Start typing the suburb or postcode and select from
                          the list.
                        </p>
                  ...
                  {step === 3 && (
                    <>
                      ...
                      <div className="mb-8">
                        <label className="block font-bold text-lg mb-1 text-[#2954bd]">
                          Job Title
                        </label>
                        <p className="text-gray-700 mb-2">
                          Summarise the support activities you want e.g. 'Help a
                          female teenager get ready for school and share a
                          passion for Star Wars!'
                        </p>
                        <input
                          className={`w-full border rounded-md p-3 text-black text-base mb-1 ${
                            touched.title && !title
                              ? "border-red-500"
                              : "border-gray-300"
                          }`}
                        ...
                        />
                        {touched.title && !title && (
                          <div className="text-red-600 text-sm flex items-center gap-1 mb-1">
                      ...
                      <div className="mb-8">
                        ...
                        <p className="text-gray-700 mb-2">
                          Describe key activities and requirements in detail.
                        </p>
                        <textarea
                          className={`w-full border rounded-md p-3 text-black text-base mb-1 resize-none min-h-[120px] ${
                        ...
                        {touched.description && description.length < 10 && (
                          <div className="text-red-600 text-sm flex items-center gap-1 mb-1">
                            <span>⚠️</span> Please enter a description
                          </div>
                        )}
                      </div>
                      {/* Tips block */}
                      <div className="bg-gray-100 border rounded-lg p-5 mb-8 flex flex-col gap-2 text-gray-800 max-w-md mx-auto">
                        <div className="flex items-center gap-2 font-semibold text-base">
                          <span>💡</span> Tips for writing support details
                        </div>
                        <ul className="list-disc pl-5 text-sm mt-1">
                          <li>
                            ...
                            matches will be. Workers can't read your profile
                            until you enter an agreement with them.
                          </li>
                          <li>
                            <span className="font-semibold">
                              Read the examples
                            </span>{" "}
                            – Below each text box are example responses to use
                            as a guide.
                          </li>
                          <li>
                            <span className="font-semibold">
                            ...
                            – If you prefer, you can write your support details
                            in short dot points.
                          </li>
                        </ul>
                      </div>
                  ...
                  <div className="mt-10 flex flex-col gap-4 pb-28 md:pb-0">
                    <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-[#2954bd] transition-all"
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center mt-2 gap-4">
                      ...
                      <button
                        ...
                        disabled={
                          ...
                          (step === 2 &&
                            (!location || !day || !startTime || !endTime)) ||
