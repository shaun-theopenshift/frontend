...
import { PencilSquareIcon, ClockIcon, UserIcon, PhoneIcon, HomeIcon, XMarkIcon, ArrowRightOnRectangleIcon, ExclamationTriangleIcon, CheckCircleIcon } from '@heroicons/react/24/outline';
...
const BADGE_ICONS: Record<string, { icon: JSX.Element; label: string }> = {
  lgbtq: { icon: <span className="text-xl" role="img" aria-label="LGBTQIA+">🏳️‍🌈</span>, label: "LGBTQIA+ Friendly" },
  non_smoker: { icon: <span className="text-xl" role="img" aria-label="Non Smoker">🚭</span>, label: "Non-Smoker" },
  pet_friendly: { icon: <span className="text-xl" role="img" aria-label="Pet Friendly">🐾</span>, label: "Pet Friendly" },
};
const VACC_INFO: Record<string, { icon: JSX.Element; label: string }> = {
  covid_19: { icon: <span className="text-xl" role="img" aria-label="COVID-19">💉</span>, label: "COVID-19 vaccine - Self declared" },
  flu: { icon: <span className="text-xl" role="img" aria-label="Flu">😷</span>, label: "Seasonal flu vaccine - Self declared" },
  tetanus: { icon: <span className="text-xl" role="img" aria-label="Tetanus">🩹</span>, label: "Tetanus vaccine - Self declared" },
};
...
const LANGUAGE_ICONS: Record<string, JSX.Element> = {
  english: <span className="text-xl" role="img" aria-label="English">🇬🇧</span>,
  spanish: <span className="text-xl" role="img" aria-label="Spanish">🇪🇸</span>,
  french: <span className="text-xl" role="img" aria-label="French">🇫🇷</span>,
  german: <span className="text-xl" role="img" aria-label="German">🇩🇪</span>,
  chinese: <span className="text-xl" role="img" aria-label="Chinese">🇨🇳</span>,
  other: <span className="text-xl" role="img" aria-label="Other">🌐</span>,
};
const INTEREST_ICONS: Record<string, JSX.Element> = {
  cooking: <span className="text-xl" role="img" aria-label="Cooking">👩‍🍳</span>,
  movies: <span className="text-xl" role="img" aria-label="Movies">🎬</span>,
  pets: <span className="text-xl" role="img" aria-label="Pets">🐶</span>,
  sports: <span className="text-xl" role="img" aria-label="Sports">🏅</span>,
  gardening: <span className="text-xl" role="img" aria-label="Gardening">🌱</span>,
  music: <span className="text-xl" role="img" aria-label="Music">🎵</span>,
  photography: <span className="text-xl" role="img" aria-label="Photography">📷</span>,
  travel: <span className="text-xl" role="img" aria-label="Travel">✈️</span>,
  art: <span className="text-xl" role="img" aria-label="Art">🎨</span>,
  reading: <span className="text-xl" role="img" aria-label="Reading">📚</span>,
  games: <span className="text-xl" role="img" aria-label="Games">🎲</span>,
  festivities: <span className="text-xl" role="img" aria-label="Festivities">🎉</span>,
  fitness: <span className="text-xl" role="img" aria-label="Fitness">💪</span>,
};
const PREFERENCE_ICONS: Record<string, JSX.Element> = {
  non_smoker: <span className="text-xl" role="img" aria-label="Non-smoker">🚭</span>,
  no_pets: <span className="text-xl" role="img" aria-label="No pets">🚫🐾</span>,
  male_only: <span className="text-xl" role="img" aria-label="Male only">👨</span>,
  female_only: <span className="text-xl" role="img" aria-label="Female only">👩‍</span>,
};
...
export default function StaffPage() {
  ...
  const steps = [
    {
      id: 'availability',
      name: 'Availability',
      description: 'Setting your availability helps organizations find you for shifts.',
      href: '/profile/staff/edit',
    },
    {
      id: 'stripe-onboarding',
      name: 'Stripe Onboarding',
      description: 'Complete Stripe onboarding to receive payments for your services. This is crucial for financial transactions.',
      href: '/profile/staff/edit',
    },
    {
      id: 'email-verification',
      name: 'Email Verification',
      description: 'Verify your email address to ensure secure communication and account integrity.',
      href: '/profile/staff/edit',
    },
    {
      id: 'bgv',
      name: 'Background Verification (BGV)',
      description: 'A background check is required for all staff members to ensure safety and trust within the community.',
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] font-ubuntu flex flex-col relative overflow-hidden">
      ...
      <div className="md:pl-72 pt-16 flex-grow relative z-10"> {/* Ensure content is above the background element */}
        <div className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto py-8">
          ...
          <div className="relative bg-gradient-to-br from-[#3464b4] to-[#2a559c] p-8 sm:p-10 rounded-3xl shadow-xl text-white mb-12 flex flex-col sm:flex-row items-center justify-between">
              <div className="flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left">
                  <div className="flex-shrink-0 relative w-32 h-32 sm:w-40 sm:h-40 rounded-full overflow-hidden border-4 border-white shadow-lg bg-white flex items-center justify-center">
                      {user?.picture ? (
                          <Image src={user.picture} alt={profile?.fname || 'User'} layout="fill" objectFit="cover" className="rounded-full" />
                      ) : (
                  ...
                  <div className="mt-4 sm:mt-0">
                      <h1 className="text-4xl sm:text-5xl font-extrabold mb-2 leading-tight">
                          <SplitText
                              text={`Hello, ${profile ? `${profile.fname}` : user?.name || 'User'}!`}
                              delay={50}
                              duration={0.8}
                              ease="power3.out"
                              splitType="chars"
          ...
          {!allStepsCompleted && (
            ...
            >
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-red-700">Profile Incomplete!</h3>
                  <p className="text-sm">Please complete the following steps to unlock full features.</p>
                </div>
              </div>
                  ...
                  return (
                    <li key={step.id} className="flex items-center gap-3">
                      <span className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                        isStepCompleted ? 'bg-green-500 text-white' : 'bg-gray-300 text-gray-700'
                      }`}>
                        {isStepCompleted ? <CheckCircleIcon className="h-4 w-4" /> : ' '}
                      </span>
                      <div className="flex-1 flex justify-between items-center">
                        <p className={`font-semibold text-base ${isStepCompleted ? 'line-through text-gray-500' : 'text-gray-800'}`}>
                          {step.name}
                        </p>
                        <Link href={step.href} className="text-[#3464b4] hover:text-[#2a559c] font-semibold text-sm transition-colors flex items-center">
          ...
          {allStepsCompleted && (
            ...
            >
              <div className="flex items-center gap-4">
                <CheckCircleIcon className="h-8 w-8 flex-shrink-0 text-green-600" aria-hidden="true" />
                <div>
                  <h3 className="text-xl font-bold text-green-700">Profile Complete!</h3>
                  <span className="text-sm">You are all set to start finding shifts!</span>
                </div>
              </div>
            </motion.div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <motion.div
              ...
              whileHover="hover"
            >
              <h3 className="text-2xl font-bold text-[#3464b4] mb-3 border-b pb-2 border-gray-100">About Me</h3>
            ...
            <motion.div
              ...
              whileHover="hover"
            >
              <h3 className="text-2xl font-bold text-[#3464b4] mb-3 border-b pb-2 border-gray-100">Skills & Services</h3>
              ...
              {(!profile?.skills || profile.skills.length === 0) && (!profile?.services || profile.services.length === 0) && (
                <span className="italic text-gray-400">No skills or services provided.</span>
              )}
            </motion.div>
            ...
            <motion.div
              ...
              whileHover="hover"
            >
              <h3 className="text-2xl font-bold text-[#3464b4] mb-3 border-b pb-2 border-gray-100">Health & Preferences</h3>
              <div className="mb-6">
                  ...
                  {profile?.badges && profile.badges.length > 0 ? (
                    profile.badges.map(badge =>
                      BADGE_ICONS[badge] ? (
                        <motion.span
                          key={badge}
                          className={`inline-flex items-center gap-2 px-4 py-2 rounded-full ${badge === 'lgbtq' ? 'bg-[#f07057]/10 text-[#f07057]' : 'bg-[#3464b4]/10 text-[#3464b4]'} text-sm font-medium`}
                          variants={blobVariants}
                          initial="hidden"
                          animate="visible"
                        >
              ...
              <div className="mb-6">
                <h4 className="text-lg font-semibold text-gray-800 mb-2">Immunisation</h4>
                {profile?.vaccinations && profile.vaccinations.length > 0 ? (
                  profile.vaccinations.map(vax =>
                    VACC_INFO[vax] ? (
                      <div key={vax} className="flex items-center gap-2 text-gray-700 text-base mb-2">
                        {VACC_INFO[vax].icon}
              ...
              {(!profile?.badges || profile.badges.length === 0) && (!profile?.vaccinations || profile.vaccinations.length === 0) && (!profile?.preferences || profile.preferences.length === 0) && (
                <span className="italic text-gray-400">No health or preference information available.</span>
              )}
            </motion.div>
            ...
            <motion.div
              ...
              whileHover="hover"
            >
              <h3 className="text-2xl font-bold text-[#3464b4] mb-3 border-b pb-2 border-gray-100">Languages & Interests</h3>
              {profile?.languages && profile.languages.length > 0 && (
                <div className="mb-6">
                  <h4 className="text-lg font-semibold text-gray-800 mb-2">Languages</h4>
                  <div className="flex flex-wrap gap-3">
              ...
              {profile?.interests && profile.interests.length > 0 && (
                <div>
                  <h4 className="text-lg font-semibold text-gray-800 mb-2">Interests & Hobbies</h4>
                  <div className="flex flex-wrap gap-3">
              ...
              {(!profile?.languages || profile.languages.length === 0) && (!profile?.interests || profile.interests.length === 0) && (
                <span className="italic text-gray-400">No language or interest information available.</span>
              )}
            </motion.div>
            ...
            <motion.div
              ...
              whileHover="hover"
            >
              <h3 className="text-2xl font-bold text-[#3464b4] mb-3 border-b pb-2 border-gray-100">Emergency Contact</h3>
              <div className="grid grid-cols-1 gap-y-2 text-gray-700 text-base">
                <div><span className="font-semibold text-gray-800">Name:</span> {profile?.emergency_contact || <span className="italic text-gray-400">N/A</span>}</div>
                <div><span className="font-semibold text-gray-800">Phone:</span> {profile?.emergency_contact_phone || <span className="italic text-gray-400">N/A</span>}</div>
              </div>
              <motion.button
                onClick={() => router.push('/profile/staff/edit')}
            ...
            <motion.div
              ...
              whileHover="hover"
            >
              <h3 className="text-2xl font-bold text-[#3464b4] mb-3 border-b pb-2 border-gray-100">Experience</h3>
        ...
        </div> {/* End px-4 sm:px-6 lg:px-8 */}
      </div> {/* End md:pl-72 pt-16 */}
      ...
      {showCompletionModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          ...
          >
            ...
            <h3 className="text-2xl font-bold text-green-600 mb-4">Profile Complete! 🎉</h3>
            <p className="text-gray-700 mb-6">
              Congratulations! Your profile is now complete and ready for finding shifts.
            </p>
            <button
